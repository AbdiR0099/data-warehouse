...
BEGIN
	...
	BEGIN TRY
		...
		TRUNCATE TABLE bronze.crm_cust_info; -- TRUCATE & INSERT METHOD // Clean the table and then add the data
		...
		PRINT '- Total BATCH LOAD DURATION: ' + CAST(DATEDIFF(second,@start_time_batch,@end_time_batch) AS NVARCHAR) + ' seconds'
		PRINT '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>';
	END TRY
...
EXEC bronze.load_bronze -- Execute Stored Procedure
